...
Node::Node (ORB_SLAM2::System::eSensor sensor, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) {
  ...
  node_handle_.param<std::string>(name_of_node_+ "/frame_id", base_frame_id_param_, camera_frame_id_param_);
...
void Node::PublishPositionAsTransform (cv::Mat position) {
  ...
  tf_broadcaster.sendTransform(tf::StampedTransform(transform, current_frame_time_, map_frame_id_param_, base_frame_id_param_));
}
...
tf::Transform Node::TransformFromMat (cv::Mat position_mat) {
  ...
  tf::Transform cam_pos(tf_camera_rotation, tf_camera_translation);
  tf::StampedTransform cam_to_bl;
  tf::Transform output;
  listener_.lookupTransform(base_frame_id_param_, camera_frame_id_param_, ros::Time(0), cam_to_bl);

  output = cam_pos*cam_to_bl.inverse();
  output.getOrigin().setZ(output.getOrigin().z() + cam_to_bl.getOrigin().z() + 0.175);
...
sensor_msgs::PointCloud2 Node::MapPointsToPointCloud (std::vector<ORB_SLAM2::MapPoint*> map_points) {
  ...
  for (unsigned int i=0; i<cloud.width; i++) {
    if (map_points.at(i)->nObs >= min_observations_per_point_) {
      ...
      data_array[2] = -1.0* map_points.at(i)->GetWorldPos().at<float> (1) + cam_to_bl.getOrigin().z() + 0.175; //z. Do the transformation by just reading at the position of y instead of z
